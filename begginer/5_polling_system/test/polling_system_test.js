 ...
 */
contract("PollingSystem", ([owner, addr1, addr2, addr3, addr4]) => {

  describe('create poll', () => {
    it("should fail if no options", async () => {
      await expectRevert(PollingSystem.new([], 60, {from: owner}), "need at least 2 options");
    });

    it("should fail if only one option", async () => {
      await expectRevert(PollingSystem.new(["1"], 60, {from: owner}), "need at least 2 options");
    });

    it("should fail if not enough time", async () => {
      await expectRevert(PollingSystem.new(["1", "2"], 59, {from: owner}), "60 seconds for voting is minimum");
    });
  });
  ...
  describe("voting", () => {
    ...
    before(async () => {
      instance = await PollingSystem.new(["1", "2", "3", "4"], 10000, {from: owner})
    })
    ...
    it('several winners', async () => {
      let instance = await PollingSystem.new(["1", "2", "3", "4"], 10000, {from: owner})
